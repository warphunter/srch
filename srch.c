...
static int usage()
{
	...
	printf("\t    [-m <maxdepth>|<mindepth>-[<maxdepth>]] [-x] [-z] [-j] [-0] [-w] [-H] [-r <cmd>] [-v <count>]\n");
	...
	printf("\t\t * This also equals '%s -ds0', but is much faster because of fewer lstat(2) calls.\n", progname);
...
#    else
	setlocale(LC_ALL, "");
	while ((ch = getopt(argc, argv, "ht:I:e:E:n:N:i:afdlbcpkm:wr:v:D:F:M:A:L:B:R:o:O:P:y:Y:W:xzZju:U:g:G:0qQs:STVXH")) != -1)
#    endif
	switch (ch) {
		case 't':
			if (*optarg == '*')
