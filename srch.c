...
static unsigned statcount = 0;		// - counter for lstat calls
    ...
    MAYBE_UNUSED static pthread_mutex_t statcount_unexp_lock = PTHREAD_MUTEX_INITIALIZER; // - for protecting statcount_unexp
#endif
...
static off_t accum_du = 0;		  // - sum of diskusage of files seen
...
struct dirlist {
	...
	off_t		 du;		  // - sum of disk usage of files in this dir and inline processed subdirs
...
MAYBE_UNUSED static pthread_mutex_t sleeping_thread_cnt_lock = PTHREAD_MUTEX_INITIALIZER; // - for protecting "sleeping_thread_cnt"
...
#     endif
	...
	if (cntbiggestdirs > 0) {
		...
		if (! heap_push(elem, ASCEND))
			heap_free_elem(elem);
	} else if (zerosized && curdir->filecnt == 0) {
		if (end_with_null)
			printf("%s%c", curdir->dirpath, '\0');
		else if (modtimelist) {
			time_t mtime = get_mtime(curdir->dirpath);
...
#		      if ! defined(__MINGW32__)
			st.st_mode = st.st_mtime = st.st_size = st.st_blocks = 0;
#		      else
			st.st_mode = st.st_mtime = st.st_size = 0;
#                     endif
			st.st_uid = st.st_gid = -1;
			st.st_nlink = 1;
			pthread_mutex_lock(&perror_lock);
...
static int usage()
{
	...
	printf("-n <re1|re2|...> Search for file names matching regular expression re1 OR re2 etc.\n");
	...
	printf("\t\t * Only one -n option is supported.\n\n");
        ...
        printf("-i <re1|re2|...> Same as -n, but perform case insensitive pattern matching.\n");
        printf("\t\t * If both -n and -i are given, the last one on the command line takes presedence.\n\n");

	printf("-N <name>\t Search for file names matching shell pattern *name* where case is ignored.\n");
...
#    if defined(__MINGW32__)
	while ((ch = getopt(argc, argv, "ht:I:e:E:n:N:i:afdm:wv:D:F:M:A:L:B:R:o:O:y:Y:xzju:U:0qs:STV")) != -1)
#    else
	setlocale(LC_ALL, "");
	while ((ch = getopt(argc, argv, "ht:I:e:E:n:N:i:afdlbcpkm:wv:D:F:M:A:L:B:R:o:O:y:Y:xzju:U:g:G:0qQs:STVXH")) != -1)
#    endif
	switch (ch) {
		case 't':
			threads = atoi(optarg);
...
#		      endif
		...
		case 'z':
			zerosized = TRUE;
			if (wc) {
				fprintf(stderr, "Option -z can not be combined with -w.\n");
				exit(1);
			}
			just_count = TRUE;
			break;
		...
		case 'o':
			...
			if (youngerthan) {
				if (olderthan <= youngerthan) {
					fprintf(stderr, "Argument to -o (%i) must be less than argument to -y / -Y\n",
						atoi(optarg));
					exit(1);
				}
		...
		case 'y':
			...
			if (olderthan) {
				if (olderthan <= youngerthan) {
					fprintf(stderr, "Argument to -y (%i) must be greater than argument to -y / -Y\n",
						atoi(optarg));
					exit(1);
				}
				older_and_younger = TRUE;
...
#		      endif
	...
	if (summarize_diskusage) {
		off_t KiB = accum_du / 2;
		if (KiB < 1024)
			printf("%-.1fK", (double) KiB);
		else if (KiB < 1024 * 1024)
			printf("%-.1fM", (double) KiB/1024);
		else if (KiB < 1024 * 1024 * 1024)
			printf("%-.1fG", (double) KiB/1024/1024);
		else
			printf("%-.1fT", (double) KiB/1024/1024/1024);
		for (i = 0; i < startdircount; i++) {
