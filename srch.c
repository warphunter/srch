...
static boolean wc = FALSE;                // - for option -w; works as wc -l; using -w is faster than piping output to wc
...
static char *cmd = NULL;		  // - assigned if option -r is specified
static pthread_mutex_t cmd_lock = PTHREAD_MUTEX_INITIALIZER; // - only one thread should manipulate cmd at a time
static boolean run_cmd = FALSE;		  // - set if option -r is specified

static time_t last_t = 0;		  // - previous timestamp in seconds since EPOCH, used in pthread_routine()
static ino_t inode = 0;			  // - set if option -J is specified
...
static boolean output_lstat_errors = TRUE;// - set to FALSE if option -C is given - default is to print out errors to stderr
...
static pthread_mutex_t modtimelist_lock = PTHREAD_MUTEX_INITIALIZER; // - printf's need to be protected
...
unsigned		mindepth     = 0;	  // - min directory depth, if option -md- is specified
...
#     endif
	...
	if (cntbiggestdirs > 0) {
		...
		if (! heap_push(elem, ASCEND))
		   ...
		   && (! mindepth || curdir->depth-1 >= mindepth)) {
		if (run_cmd)
			do_run(cmd, curdir->dirpath);
		else if (end_with_null)
			printf("%s%c", curdir->dirpath, '\0');
		else if (modtimelist) {
			char *timestr = printable_time(curdir->modtime);
			pthread_mutex_lock(&modtimelist_lock);
			printf("%s %s\n", timestr, curdir->dirpath);
			pthread_mutex_unlock(&modtimelist_lock);
			free(timestr);
		} else
			puts(curdir->dirpath);
	}
...
#     endif
	...
	if (dent->d_type == DT_DIR
	    || (dent->d_type == DT_UNKNOWN && (curdir->st_nlink > 2 || filetypemask))
	    || lstat_needed) {
...
#	      else // __MINGW32__ follows:
		...
		if (handle == INVALID_HANDLE_VALUE) {
			...
			if (output_lstat_errors) {
				pthread_mutex_lock(&perror_lock);
				fprintf(stderr, "%s: FindFirstFile() failed on %s\n", progname, path);
				pthread_mutex_unlock(&perror_lock); 
			}
		} else {
			FindClose(handle);

			st.st_mode = ffd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY ? S_IFDIR : S_IFREG;
...
#             endif
	...
	if ((! mindepth || curdir->depth >= mindepth)
	    && filename_match(match_all_path_elems ? path : dent->d_name)
	    && (
		! filetypemask
...
static int usage()
{
	...
	printf("\t    [-f] [-d] [-m <maxdepth>|<mindepth>-[<maxdepth>]] [-x] [-z] [-j] [-0] [-w] [-H] [-r <cmd>] [-v <count>]\n");
	...
	printf("\t    [-I <count>] [-q | -Q] [-C] [-S] [-T] [-V] [-h] [arg1 [arg2] ...]\n\n");
#else
	printf("Usage: %s [-t <count>|*] [[-n|-i [!]<re1|re2|...> | -N [!]<name>] [-a]] [-e <dir> ... | -E <dir> ... | -Z]\n", progname);
	printf("\t    [-f] [-d] [-l] [-b] [-c] [-p] [-k]\n");
	printf("\t    [-m <maxdepth>|<mindepth>-[<maxdepth>]] [-x] [-z] [-j]\n");
	printf("\t    [-J <inode#>|<path>] [-0] [-w] [-H] [-r <cmd>] [-v <count>]\n");
	printf("\t    [-u <user> ... | -U <user> ...] [-g <group> ... | -G <group> ...]\n");
	printf("\t    [-o <days> | -O <minutes> | -P <tstamp-file>] [-y <days> | -Y <minutes>] | -W <tstamp-file>]\n");
	printf("\t    [-s [+|-]<size>[k|m|g|t] | +<size>[k|m|g|t]:-<size>[k|m|g|t]]\n");
	printf("\t    [-D <count> | -F <count> | -M <count> | -A <count> | -L <count> | -B <count> | -R <count>]\n");
	printf("\t    [-I <count>] [-q | -Q] [-X] [-C] [-S] [-T] [-V] [-h] [arg1 [arg2] ...]\n\n");
#endif
	printf("-t <count>|*\t Run up to <count> or '*' threads in parallel.\n");
	printf("\t\t * <count> must be a non-negative integer between 1 and %i.\n", MAX_THREADS);
	printf("\t\t * '*' means all available CPU cores.\n");
	printf("\t\t * Defaults to (virtual) CPU count on host, up to 8.\n");
	printf("\t\t * Note that <count> threads will be created in addition to the main thread, so the total number\n");
	printf("\t\t   of threads utilized, will be <count+1> or *+1.  The main thread will be mostly idle.\n\n");
	...
	printf("\t\t -m <maxdepth> : Descend at most <maxdepth> levels below the start point(s).\n");
	printf("\t\t -m <mindepth>- : Descend at least <mindepth> levels below the start point(s).\n");
	printf("\t\t -m <mindepth>-<maxdepth> : Descend between <mindepth> and <maxdepth> directory levels.\n");
	printf("\t\t * This is similar to the -maxdepth and -mindepth options to GNU find(1).\n\n");
	...
	printf("\t\t * Apart from the exclude and age options, -z can only be combined with a few others, like -m/-x/-j/-0/-r.\n\n");
...
#if ! defined(__MINGW32__)
	printf("-J <inode#>|<path>\n");
	printf("\t\t * Search for specified inode number, or files with same inode number as <path>.\n");
	printf("\t\t * Using the <path> option equals the -sametime option to GNU find(1).\n\n");
#endif
	...
	printf("\t\t   to delete the matched files.\n\n");
        ...
        printf("-H\t\t Print out the sum of the file/directory sizes, in powers of 1024, of all the files/directories encountered.\n");
        printf("\t\t * This equals `du -hs'.\n\n");

        printf("-r <cmd>\t For each matched file/directory, execute the shell command <cmd>.\n");
	printf("\t\t * If <cmd> contains a space, it must be escaped by a '\\', or <cmd> must be enclosed by single or double quotes.\n");
	printf("\t\t * A character pair `{}' within <cmd>, is replaced by the current file path.\n");
	printf("\t\t * Note that just up to 2 occurences of the `{}' pair are replaced by the file path.\n");
	printf("\t\t * If more than 2 are needed, a variable f=`{}' can be introduced for one of the two pairs.\n");
	printf("\t\t * If no `{}' pair is found in <cmd>, the file path is appended at the end of <cmd>, enclosed by single quotes.\n");
	printf("\t\t * Using quotes around a brace pair is recommended, so that file names containing spaces are correctly handled.\n");
	printf("\t\t * If file path is not needed at the end of <cmd>, adding ' #' or ';:' to <cmd>, makes it invisible.\n");
	printf("\t\t * This option may be CPU intensive as it creates a new process for each file/directory matched,\n");
	printf("\t\t   by default using 8 threads in parallel, or less if there are fewer cores.\n\n");
	...
	printf("-C\t\t Clear/skip error messages about files/directories that don't exist any more.\n");
	printf("\t\t * Default is to print an error message when a file/directory has been found,\n");
	printf("\t\t   but has been deleted from the file system before lstat() was called.\n\n");
	...
	printf("* The primary location for the latest version of the '%s' program is https://github.com/warphunter/srch\n", progname);
	...
	char *argv[])
{
	...
	boolean timer = FALSE;
	unsigned threads, cpu_cnt = 1;
#    if defined(__hpux)
	struct pst_dynamic psd;

	if (pstat_getdynamic(&psd, sizeof(psd), (size_t)1, 0))
		cpu_cnt = (unsigned) psd.psd_proc_cnt;
#    elif defined(__MINGW32__)
	SYSTEM_INFO sysinfo;
	GetSystemInfo(&sysinfo);
	cpu_cnt = sysinfo.dwNumberOfProcessors;
#    else
	cpu_cnt = (unsigned) sysconf(_SC_NPROCESSORS_ONLN);
#    endif
	threads = cpu_cnt > 8 ? 8 : cpu_cnt; // - using 8 as default max number of threads
...
#    if defined(__MINGW32__)
	while ((ch = getopt(argc, argv, "ht:I:e:E:n:N:i:afdCm:wv:r:D:F:M:A:L:B:R:o:O:P:y:Y:W:xzju:U:0qQs:STVH")) != -1)
#    else
	setlocale(LC_ALL, "");
	while ((ch = getopt(argc, argv, "ht:I:e:E:n:N:i:afdlbcpkCm:wr:v:D:F:M:A:L:B:R:o:O:P:y:Y:W:xzZjJ:u:U:g:G:0qQs:STVXH")) != -1)
#    endif
	switch (ch) {
		case 't':
			if (*optarg == '*')
		...
		case 'e':
			if (E_option) {
				fprintf(stderr, "Option -%c can not be combined with -E.\n", ch);
				exit(1);
			}
		...
		case 'f':
			if (zerosized) {
				fprintf(stderr, "Option -%c can not be combined with -z.\n", ch);
				exit(1);
			}
...
#		      endif
			break;
		case 'C':
			output_lstat_errors = FALSE;
			break;
		case 'm':
			dash = strchr(optarg, '-');
			if (! dash) { // - no mindepth given
				if (atoi(optarg) < 1)
					return usage();
				maxdepth = atoi(optarg);
			} else { // - mindepth given
				if (*(dash+1)) { // - maxdepth given
					*dash = '\0';
					mindepth = atoi(optarg);
					maxdepth = atoi(dash+1);
				} else { // - no maxdepth given
					*dash = '\0';
					mindepth = atoi(optarg);
				}
			}
		...
		case 'r':
			if (! *optarg || strlen(optarg) < 2)
				return usage();
			if (strlen(optarg) > LINE_MAX/2) {
				fprintf(stderr, "-r: Command length overflow - max is %i - bailing out\n", (int) LINE_MAX/2);
				exit(1);
			}
			cmd = optarg;
			run_cmd = TRUE;
		...
		case 'z':
			zerosized = TRUE;
			if (wc || regex_opt || fast_match_opt || filetypemask || filesizesearch
			    || cntbiggestdirs || timestamp_or_size_on_heap || cntlongestdepth
			    || uid_or_gid) {
				fprintf(stderr, "Option -%c can not be combined with -w, -n, -N, -i etc.\n", ch);
				exit(1);
			}
			just_count = TRUE;
			break;
		...
		case 'u':
			...
			if (uidnegsearch) {
				fprintf(stderr, "Option -%c can not be combined with -U.\n", ch);
				exit(1);
			}
		...
		case 'g':
			...
			if (gidnegsearch) {
				fprintf(stderr, "Option -%c can not be combined with -G.\n", ch);
				exit(1);
			}
		...
		case 'G':
			...
			break;
		case 'J':
			if (atoi(optarg) < 1) { // - assume <path> is given
				if (lstat(optarg, &st)) {
                                	perror(optarg);
                                	exit(1);
				}
		...
		case 'j':
			if (timestamp_or_size_on_heap || cntbiggestdirs > 0 || cntlongestdepth > 0) {
				fprintf(stderr, "Ignoring option -j which can not be combined with -M, -A, -L, -B, -D, -R.\n");
				break;
			}
			modtimelist = TRUE;
			break;
		...
		case 'M':
			if (modtimelist) {
				fprintf(stderr, "Ignoring option -j which can not be combined with -M.\n");
				modtimelist = 0;
			}
			if (timestamp_or_size_on_heap || cntbiggestdirs > 0 || cntlongestdepth > 0) {
		...
		case 'A':
			if (modtimelist) {
				fprintf(stderr, "Ignoring option -j which can not be combined with -A.\n");
				modtimelist = 0;
			}
			// option -j is okay here - it will just be ignored...
			if (timestamp_or_size_on_heap || cntbiggestdirs > 0 || cntlongestdepth > 0) {
		...
		case 's':
			if (zerosized) {
				fprintf(stderr, "Option -s can not be combined with -z.\n");
				exit(1);
			}
...
#                             	      endif
	...
	thread_cnt = threads; // thread_cnt is used globally
	thread_prepare();
